...
def _migrate_db():
    """
    No Postgres, criamos as tabelas caso não existam.
    (Se você já criou no Supabase via SQL Editor, isso só garante.)
    """
    conn = get_conn()
    cur = conn.cursor()
...
def salvar_ou_atualizar_solicitacao(
    ...
    cur.execute(
        """
        UPDATE public.casos
        SET assunto_solic = %s, prazo_om = %s, status = 'Distribuído', nr_doc_solicitado = %s
        WHERE id = %s
        """,
        (
